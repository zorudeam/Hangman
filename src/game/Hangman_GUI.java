...
public class Hangman_GUI extends JFrame {
    ...
    /**
     * Method that should only be called in the constructor. Adds action 
     * listeners for keyboard input to each relevant component from the given 
     * {@code JPanel}.
     */
    ...
    private void initComponents() {
        ...
        actorComboBox.setModel(new javax.swing.DefaultComboBoxModel(Actor.getNames()));
        ...
        menuBarFile_New.setText("New Game");
        menuBarFile_New.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                menuBarFile_NewActionPerformed(evt);
            }
        });
        ...
        menuBarEdit_Settings.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                menuBarEdit_SettingsActionPerformed(evt);
            }
        });
    ...
    private void menuBarEdit_SettingsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuBarEdit_SettingsActionPerformed
        updateDisplayList();
        showSettingsFrame();
    }//GEN-LAST:event_menuBarEdit_SettingsActionPerformed
    ...
    private void menuBarFile_NewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuBarFile_NewActionPerformed
        updateDisplayList();
        showSettingsFrame();
    }//GEN-LAST:event_menuBarFile_NewActionPerformed
    ...
    /**
     * Method that attempts to get user input on whether or not to "give up" or
     * throw this current game. If the user specifies yes, the current game is
     * considered a loss and the user is shown the correct word. Otherwise, 
     * nothing happens.
     */
    ...
    /**
     * Method for displaying the {@code aboutFrame} JFrame.
     */
    ...
    /**
     * Method for displaying the {@code addFrame} JFrame.
     */
    ...
    /**
     * Method for displaying the {@code settingsFrame} JFrame.
     */
    ...
    /**
     * Method for parsing and handling the value a key event. If the key pressed
     * is not between {@code 'a'} through {@code 'z'}, do nothing. Otherwise go
     * on to find its representative {@code JButton} and disable it.
     * 
     * @param value The value of the key that was pressed.
     */
    private void typedLetter(KeyEvent evt) {
        char guess = Character.toLowerCase(evt.getKeyChar());
        makeMove(guess);
    }
    
    /**
     * Method that finds the first {@code JButton} in the {@code keyboardPanel} 
     * that represents the given character parameter and disables it.
     * 
     * @param guess The character that represents the guess.
     */
    private void disableRepresentative(char guess) {
        guess = Character.toLowerCase(guess);
        for (int i = 0; i < keyboardPanel.getComponentCount(); i++) {
            JButton current = (JButton) keyboardPanel.getComponent(i);
            if (current.getText().toLowerCase().contains(String.valueOf(guess))) {
    ...
    /**
     * Method that disables all components relating to the keyboard panel.
     */
    private void resetKeyboard(boolean state) {
        setEnabled(keyboardPanel, state);
    }
    
    /**
     * Method that initializes the game board and all the panels relating to it.
     */
    private void initGame() {
        ensureDifficulty();
        updateActor();
        
        String formatted = StringUtilities
                .expandString(gameManager.getCorrectGuesses(), " ");
    ...
    /**
     * Method that updates the current actor, which handles the image display on
     * the main game panel, to the appropriate, user-specified actor.
     */
    private void updateActor() {
        String[] defaultActors = Actor.getNames();
        String selected = actorComboBox.getSelectedItem().toString();
        for (int i = 0; i < defaultActors.length; i++) {
            if (defaultActors[i].equals(selected)) {
                gameManager.setActor(Actor.values()[i]);
                break;
            }
        }
    }
    ...
    /**
     * Method that sets the state of all the game panels in the main form to the
     * given {@code boolean} state.
     * 
     * @see   java.awt.Component#setEnabled(boolean)
     * @see   #setEnabled(boolean)
     * @param state The {@code boolean} state to put the panels in.
     */
    private void resetPanels(boolean state) {
        setEnabled(gamePanel, state);
        setEnabled(statisticsPanel, state);
        setEnabled(keyboardPanel, state);
        setEnabled(currentPanel, state);
        setEnabled(gameOptionsPanel, state);
    }
    
    /**
     * Method that updates the statistics display panel.
     */
    private void updateStatistics() {
        String guessed = StringUtilities
                .breakUpString(gameManager.getAlreadyGuessed().toUpperCase());
        guessedField.setText(guessed);
        
        //Subtract 1 to prevent index out of bounds exception.
        int remaining = gameManager.getActor()
                .getImageArray().length - gameManager.getGuessesMade() - 1;
    ...
    /**
     * Method that attempts to make a move on the game board. If a move attempt
     * is valid, update the game board appropriately.
     * 
    ...
    private void makeMove(char guess) {
        ...
        pushMoveEvents(guess);
        disableRepresentative(guess);
    }
    
    /**
     * Method that checks for the validity of any move.
     * 
     * @param guess The character guess to check for validity.
     * @return      Returns {@code true} if the given character is a valid 
     *              guess, {@code false} if not.
     */
    private boolean validGuess(char guess) {
        if (gameManager.getGuessesMade() >= gameManager.getMaxGuesses()) {
            return false;
        }
        guess = Character.toLowerCase(guess);
        if (guess < 'a' || guess > 'z') {
        ...
        return !gameManager.getAlreadyGuessed().contains(String.valueOf(guess));
    }
    
    /**
     * Method that pushes the events of a guess.
     * 
     * @param guess The character to attempt to guess.
     */
    private void pushMoveEvents(char guess) {
        guess = Character.toLowerCase(guess);
        boolean valid = gameManager.guessLetter(guess);
        if (valid) {
            validMove();
        }
        else {
    ...
    private void checkGameState() {
        if (gameManager.hasWon()) {
            winnerState();
        } else if (gameManager.getGuessesMade() >= gameManager.getMaxGuesses()) {
            loserState();
        }
    ...
    private void winnerState() {
        ...
        endOfGameChecks("Nice guessing! \""
                + StringUtilities.formatAsSentence(gameManager.getCurrentWord())
    ...
    /**
     * Method that handles the "give up" state of the game.
     */
    ...
    private void endOfGameChecks(String message, String title) {
        String actual = StringUtilities
                .expandString(gameManager.getCurrentWord(), " ");
    ...
    /**
     * Method that updates the current set of images on the game panel.
     */
    private void updateImages() {
        int index = gameManager.getGuessesMade();
        if (index >= gameManager.getActor().getImageArray().length) {
            return;
        }
        imageLabel.setIcon(gameManager.getActor().getImageArray()[index]);
    }
    
    /**
     * Method that sets the difficulty of the random word.
     */
    private void ensureDifficulty() {
        if (easyRadioButton.isSelected()) {
            gameManager.initGame(Hangman.EASY_DIFFICULTY);
        } 
        else if (mediumRadioButton.isSelected()) {
            gameManager.initGame(Hangman.MEDIUM_DIFFICULTY);
        } 
        else if (hardRadioButton.isSelected()) {
            gameManager.initGame(Hangman.HARD_DIFFICULTY);
        }
        else {
            gameManager.initGame(Hangman.ANY_DIFFICULTY);
        }
        currentLabel.setToolTipText("The current word is " 
    ...
    private void removeItemFromList() {
        ...
        int response = showConfirmPane("Really remove \""
                + gameManager.getWords().getList().get(index)
                + "\" from the dictionary?",
                "Word Removal");
        if(response == JFileChooser.APPROVE_OPTION) {
            gameManager.getWords().remove(wordDisplayList.getSelectedIndex());
    ...
    private void addItemToList() {
        ...
        add = add.toLowerCase();
        if (!gameManager.getWords().getList().contains(add)) {
            gameManager.getWords().add(wordField.getText());
            wordField.setText("");
            wordDisplayList.updateUI();
        }
        else {
    ...
    /**
     * Method that updates the {@code JList} display of the dictionary.
     */
    private void updateDisplayList() {
        gameManager.getWords().sort();
        wordDisplayList.setModel(new javax.swing.AbstractListModel() {
            ArrayList<String> words = gameManager.getWords().getList();
    ...
    private void updateWordList(String path) {
        if (path == null) {
            ...
            Dictionary list = new Dictionary(path);
            gameManager.setWords(list);
            wordDisplayList.updateUI();
        }
    }
    ...
    /**
     * Sets a {@code Component} and all its children to {@code state} parameter 
     * status. Useful for disabling all {@code JPanels} and their contents.
     * 
     * @param array The array of {@code comp} parameter's children.
     * @param comp  The {@code Component} that contains the objecst that are to
     *              be enabled or disabled.
     * @param state The state to put the array of {@code Component} objects and 
     *              the {@code Component} itself in.
     */
    private void setEnabled(Container comp, boolean state) {
        for (int i = 0; i < comp.getComponentCount(); i++) {
            comp.getComponent(i).setEnabled(state);
        }
    ...
    private final Hangman gameManager;
